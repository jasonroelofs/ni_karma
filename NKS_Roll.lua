...
---------------------------------------
function KarmaRoll_OnLoad(frame)
...
-- Main event handler
function KarmaRoll_OnEvent(frame, event, ...)
...
function KarmaRollList_Update()
  ...
  for i=1, nks.ROLLS_TO_DISPLAY do
    ...
    button = _G["KarmaRollFrameButton"..i];
    button.rollIndex = rollIndex;
    local roll_info = nks.RollList[rollIndex];
    if (roll_info ~= nil) then
      _G["KarmaRollFrameButton"..i.."Name"]:SetText(roll_info[1]);
      _G["KarmaRollFrameButton"..i.."Class"]:SetText(roll_info[2]);
      _G["KarmaRollFrameButton"..i.."Karma"]:SetText(roll_info[3]);
      _G["KarmaRollFrameButton"..i.."Roll"]:SetText(roll_info[4]);
      _G["KarmaRollFrameButton"..i.."Total"]:SetText(roll_info[5]);
    end
...
function KarmaRollFrameRollButton_OnClick(self, button)
  if ( button == "LeftButton" ) then
    KarmaRollFrame.selectedRoller = _G["KarmaRollFrameButton"..self:GetID()].rollIndex;
    KarmaRollList_Update();
  else
    -- Remove from list
    local roller = _G["KarmaRollFrameButton"..self:GetID()].rollIndex
...
function Karma_SetClassItem(tooltip)
  if (nks.OpenRoll) then
    ...
    for n=1, numlines do
      local text_field = _G[tooltip:GetName().."TextLeft".. n];
      if (text_field) then
