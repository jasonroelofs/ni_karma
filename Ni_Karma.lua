...
function Karma_OnLoad(frame)
  ...
  Karma_OnEvent(frame, "ADDON_LOADED");
end
...
function Karma_ChatFrame_OnEvent(self,event,...)
  ...
  local cmd,extra = Karma_GetToken(message);
  cmd = Karma_StripTok(cmd);

  -- Check if enabled
  if (nks.Active) then
    if (event == "CHAT_MSG_WHISPER_INFORM" and string.find(message, "KarmaBot: ")
...
-- Main event handler
function Karma_OnEvent(frame, event, message, player)
  ...
  if (event == "CHAT_MSG_WHISPER") then
    local cmd, extra = Karma_GetToken(message);
    if (string.lower(string.sub(message,1,2)) == "km") then
      cmd = Karma_StripTok(cmd);
      Karma_Player_Request(string.lower(player), extra);
    end
  end
end
  ...
  if (select('#', ...) > 0) then
    if (select('1', ...) == nks.KARMA_SHOWTO_PLAYER) then
      ...
      for m in string.gmatch(msg, "([^\n]*)\n*") do
        if (m ~= "") then
          SendChatMessage("KarmaBot: ".. m, "WHISPER", nil, select('2', ...));
        end
      end
    elseif (select('1', ...) == nks.KARMA_SHOWTO_RAID) then
      ...
      for m in string.gmatch(msg, "([^\n]*)\n*") do
        if (m ~= "") then
          SendChatMessage("Karma: ".. m, "RAID", nil, "");
        end
      end
